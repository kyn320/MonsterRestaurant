...
namespace GDataDB.Impl
{
    public class Table<T> : ITable<T>
    {
        ...
        public IList<IRow<T>> FindAll(string path)
        {
            return Find(GetQuery(), path);
        }
        ...
        public IList<IRow<T>> Find(Query q, string path)
        {
            var fq = GetQuery();
            fq.Query = q.FreeQuery;
            fq.SpreadsheetQuery = q.StructuredQuery;
        ...
        private IList<IRow<T>> Find(FeedQuery q, string path)
        {
            var feed = (ListFeed)svc.Query(q);
            var l = new List<IRow<T>>();
            foreach (ListEntry e in feed.Entries)
            {
                l.Add(new Row<T>(e) { Element = serializer.Deserialize(e, path) });
            }
            return l;
        }
